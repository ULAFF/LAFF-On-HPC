...
void Gemm_8x6Kernel_Packed( int, double *, double *, double *, int );
...
void LoopOne( int m, int n, int k, double *Atilde, double *MicroPanelB, double *C, int ldC )
{
  for ( int i=0; i<m; i+=MR ) {
    int ib = min( MR, m-i );
    Gemm_8x6Kernel_Packed( k, &Atilde[ i*k ], MicroPanelB, &gamma( i,0 ), ldC );
  }
}
...
void Gemm_8x6Kernel_Packed( int k,
		        double *BlockA, double *PanelB, double *C, int ldC )
{
  ...
  for ( int p=0; p<k; p++ ){
    ...
    beta_p_j = _mm256_broadcast_sd( PanelB+4 );
    ...
    beta_p_j = _mm256_broadcast_sd( PanelB+5 );
